from hp_finetune import FineTune
...
hp_space = {'batch_size': hp.qloguniform('batch_size', 3, 6, 32),
            'threshold': hp.quniform('threshold', 10, 100, 10), 
            'init_lr': hp.qloguniform('init_lr', -9, -6, 1e-4),
            'init_base_lr': hp.qloguniform('init_base_lr', -10, -7, 5e-5),
            'weight_decay': hp.qloguniform('weight_decay', -14, -11, 1e-6),
            'dropout': hp.quniform('dropout', 0.0, 0.5, 0.1),
...
def objective(params):
    ...
    config['model']['pred_n_layer'] = int(params['n_layer'])
    config['model']['n_heads'] = int(params['n_heads'])
    config['model']['pred_act'] = params['activation']
