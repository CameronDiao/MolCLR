...
class FineTune(object):
    ...
    def _init_motif_prompts(self, clique_loader, train_loader, model):
        motif_feats = []
        ...
        motif_feats = torch.cat((motif_feats, emp_init), dim=0)

        return emp_init, motif_feats
    ...
    def train(self):
        ...
        print("Number of cliques/motifs: ", len(clique_list))
        print("Number of 'empty' molecules: ", len(emp_mol))

        clique_dataset = MolCliqueDatasetWrapper(deepcopy(clique_list), self.config['batch_size'], self.config['dataset']['num_workers'])
        ...
        if self.config['model_type'] == 'gin':
            ...
            label_means, motif_feats = self._init_motif_prompts(clique_loader, train_loader, model)

            label_feats = label_means.detach().clone()
        ...
        for epoch_counter in range(self.config['epochs']):
            for bn, data in enumerate(train_loader):
                ...
                with torch.no_grad():
                    ...
                    label_samples = label_embed(torch.zeros(data.num_graphs, dtype=torch.long)).unsqueeze(0).to(self.device)
                    label_samples = torch.cat((label_samples, label_embed(torch.ones(data.num_graphs, dtype=torch.long))\
            ...
            for bn, data in enumerate(valid_loader):
                ...
                with torch.no_grad():
                    ...
                    label_samples = torch.cat((label_samples, label_emb_tensor.index_select(0, torch.ones(data.num_graphs,\
                            dtype=torch.long)).unsqueeze(0).to(self.device)), dim=0)
